...
def variante_dynamique(liste_cercle_1, liste_cercle_2, couleur, Obstacle, liste_obstacle):
    """
    Permet d'augmenter le rayon de toute les boules présente sur le terrain d'un certain nombre de pixel
    les paramètres liste_cercle_1 et liste_cercle_2 représente les listes des cercles des deux joueurs
    ...
    """
    for i in range(5):
        for i in range(len(liste_cercle_1)):
            intersection = 0
            x, y, r = liste_cercle_1[i][0], liste_cercle_1[i][1], liste_cercle_1[i][2]+1
            if Obstacle:
                if detecter_intersection(x, y, r, liste_obstacle) == True:
                    intersection += 1
            for element in liste_cercle_2:
                distance = sqrt((x - element[0])**2 + (y-element[1])**2)
                if distance < element[2] + r:
                    intersection += 1

            liste = calcul_extremite_cercle(liste_cercle_1[i], r)
            if liste[0]<50 or liste[1]>1230 or liste[2]<100 or liste[3]>670:
                intersection += 1
                
            if intersection == 0:
                efface(liste_cercle_1[i][3])
                id_cercle = cercle(x, y, r, "black", couleur, tag="Jeu")
                liste_cercle_1[i] = [x, y, r, id_cercle]
    return liste_cercle_1
...
def scinder(x, y, liste_cercle_2, liste_cercle_1, element, distance, tour):
    ...
    liste_cercle_1 (list) : liste contenant les informations sur les différents cercle du joueur vert sur le terrain
    liste_cercle_2 (list) : liste contenant les informations sur les différents cercle du joueur violet sur le terrain
...
def cerkle(x, y, tour, liste_cercle_1, liste_cercle_2, rayon):
    """"
    ...
    liste_cercle_1 / liste_cercle_2 : permet de récupérer les informations des cercles des deux joueurs
